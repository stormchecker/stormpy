...
using GenerateMonitorVerifier =
    storm::generator::GenerateMonitorVerifier<ValueType>;
template <typename ValueType>
...
template <typename ValueType>
using GenerateMonitorVerifierOptions =
    typename storm::generator::GenerateMonitorVerifier<ValueType>::Options;

template <typename ValueType>
void define_verimon_generator(py::module &m, std::string const &vtSuffix) {
  py::class_<storm::generator::MonitorVerifier<ValueType>,
             std::shared_ptr<storm::generator::MonitorVerifier<ValueType>>>
  ...
  gmv.def(py::init<SparseDtmc<ValueType> const &, SparseMdp<ValueType> const &,
                   std::shared_ptr<storm::expressions::ExpressionManager> &,
                   GenerateMonitorVerifierOptions<ValueType> const &>(),
          py::arg("mc"), py::arg("monitor"), py::arg("expr_manager"),
          py::arg("options"));
  gmv.def("create_product",
          &storm::generator::GenerateMonitorVerifier<ValueType>::createProduct,
          py::return_value_policy::copy, "Created the verification POMDP");
  gmv.def("set_risk",
          &storm::generator::GenerateMonitorVerifier<ValueType>::setRisk,
          py::arg("risk"));

  py::class_<GenerateMonitorVerifierOptions<ValueType>> gmvopts(
      m, ("GenerateMonitorVerifier" + vtSuffix + "Options").c_str(),
      "Options for corresponding generator");
  gmvopts.def(py::init<>());
  gmvopts.def_readwrite("good_label",
                        &GenerateMonitorVerifierOptions<ValueType>::goodLabel);
  gmvopts.def_readwrite(
      "accepting_label",
      &GenerateMonitorVerifierOptions<ValueType>::acceptingLabel);
  gmvopts.def_readwrite("step_prefix",
                        &GenerateMonitorVerifierOptions<ValueType>::stepPrefix);
  gmvopts.def_readwrite(
      "horizon_label",
...
template void
define_verimon_generator<storm::RationalNumber>(py::module &m,
                                                std::string const &vtSuffix);
