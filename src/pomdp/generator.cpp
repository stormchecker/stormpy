...
void define_verimon_generator(py::module& m, std::string const& vtSuffix) {
    py::class_<storm::generator::MonitorVerifier<ValueType>, std::shared_ptr<storm::generator::MonitorVerifier<ValueType>>> mv(m, ("MonitorVerifier" + vtSuffix).c_str(), "Container for monitor verifier POMDP with associated objects");
    mv.def(py::init<const SparsePomdp<ValueType>&, const std::map<std::pair<uint32_t, bool>, uint32_t>&>(), py::arg("product"), py::arg("observation_map"));
    mv.def("get_product", &storm::generator::MonitorVerifier<ValueType>::getProduct, py::return_value_policy::copy);
    mv.def_property_readonly("observation_map", &storm::generator::MonitorVerifier<ValueType>::getObservationMap, py::return_value_policy::copy);
    ...
    gmv.def("create_product", &storm::generator::GenerateMonitorVerifier<ValueType>::createProduct, py::return_value_policy::copy, "Created the verification POMDP");
    ...
    gmvopts.def_readwrite("use_risk", &GenerateMonitorVerifierOptions<ValueType>::useRisk);
}
